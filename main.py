...
def ask_questions(repo_name, github_url, index, documents, file_type_counts, filenames, llm_chain, conversation_history):
    ...
    while True:
        try:
            user_question = input("\n" + WHITE + "Ask a question about the repository (type 'exit()' to quit): " +
                                  RESET_COLOR)
...
def generate_prompt_template(repo_name, github_url, conversation_history, question, numbered_documents, file_type_counts,
    ...
    Generates the prompt template for asking a question.
    """
    template = """Repo: {repo_name} ({github_url}) | Conv: {conversation_history} | Docs: {numbered_documents} | Q: {question} | FileCount: {file_type_counts} | FileNames: {filenames}
    Instr:
    1. Answer based on context/docs.
    2. Focus on repo/code.
    3. Consider:
...
Changes made:
1. Modularized the code by breaking down the functions into smaller, reusable functions with a single responsibility and clear purpose.
2. Used meaningful and descriptive variable and function names to improve code readability.
3. Added comments and docstrings where necessary to explain complex logic.
4. Formatted the code according to PEP 8 guidelines for Python.
5. Made the code more efficient by optimizing performance bottlenecks and using efficient algorithms and data structures where applicable.
6. Added appropriate error handling and exception management for robust error handling.
7. Eliminated redundant and unused code.
8. Simplified complex logic where possible.
